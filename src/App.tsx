    ...
    <div className="min-h-screen font-sans flex flex-col bg-gray-50 text-gray-800 transition-colors duration-500">
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm transition-all duration-500">
          ...
          {messagesBanniere.map((msg, idx) => (
            <span
              ...
              className={`absolute w-full px-2 transition-all duration-700 ease-in-out ${
                indexBanniere === idx ? "opacity-100 translate-y-0" : "opacity-0 translate-y-full"
              }`}
            >
              {msg}
            </span>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          {/* BOUTON RECHERCHE PREMIUM (Caché sur mobile, visible sur PC) */}
            <button 
              onClick={() => setIsSearchModalOpen(true)} 
              className="hidden lg:flex items-center gap-2 bg-gray-50 hover:bg-gray-100 text-gray-500 px-3 md:px-4 py-2 md:py-2.5 rounded-full transition-colors w-10 md:w-64 border border-gray-100 shadow-inner"
            >
                    ...
                    {panier.length === 0 ? (
                      ...
                      <>
                          ...
                          {panier.map((item, idx) => (
                            <div key={idx} className="flex items-center gap-3 border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                              ...
                              <div className="text-right shrink-0 flex flex-col items-end">
                                ...
                                <button onClick={(e) => { e.stopPropagation(); removeFromCart(item.id); }} className="text-[9px] text-gray-400 hover:text-red-600 font-bold uppercase mt-1 flex items-center gap-1 transition-colors">
                                  <svg width="10" height="10" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"></path></svg>
                                  Retirer
                                </button>
                              </div>
                            </div>
                        ...
                        <div className="bg-gray-50 p-4 border-t border-gray-100">
                          ...
                          <button onClick={() => { setView("panier"); setShowMiniCart(false); }} className="w-full bg-[#800020] hover:bg-black text-white py-3.5 rounded-xl font-black uppercase text-[11px] tracking-widest transition-colors shadow-md flex justify-center items-center gap-2">
                            Aller au panier ➔
                          </button>
                        </div>
                      </>
        ...
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex justify-center items-center gap-6 md:gap-10">
            ...
            <a href="https://wa.me/261348697298" target="_blank" rel="noreferrer" className="text-white hover:text-yellow-400 transition-all transform hover:scale-110">
              <svg width="32" height="32" fill="currentColor" viewBox="0 0 24 24"><path d="M11.996 0A12 12 0 000 12c0 2.146.568 4.16 1.558 5.885L.234 23.766l6.016-1.554A11.956 11.956 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 11.996 0zm0 22.036c-1.802 0-3.52-.469-5.053-1.309l-.36-.197-3.753.97.994-3.593-.217-.336c-.927-1.436-1.465-3.136-1.465-4.957C2.142 6.574 6.574 2.142 12 2.142S21.858 6.574 21.858 12 17.426 21.858 12 22.036zm5.823-8.083c-.319-.16-1.892-.936-2.185-1.042-.292-.107-.506-.16-.719.16-.213.32-.827 1.042-1.014 1.255-.187.213-.374.24-.694.08-.32-.16-1.35-.497-2.57-1.514-.948-.79-1.589-1.765-1.776-2.084-.187-.32-.02-.493.14-.652.143-.143.32-.374.48-.56.16-.187.213-.32.32-.534.107-.213.053-.4-.027-.56-.08-.16-.719-1.733-.985-2.373-.26-.622-.524-.537-.719-.547-.187-.01-.4-.01-.614-.01-.213 0-.56.08-.853.4s-1.12 1.093-1.12 2.666c0 1.573 1.147 3.093 1.307 3.306.16.213 2.254 3.44 5.454 4.826.76.333 1.354.533 1.819.682.766.242 1.464.208 2.015.126.618-.093 1.892-.773 2.158-1.52.266-.746.266-1.386.186-1.52-.08-.133-.293-.213-.613-.373z"/></svg>
            </a>
            {/* Email (Copie Automatique) */}
            <button 
              onClick={(e) => { 
                e.preventDefault(); 
                navigator.clipboard.writeText('hakimiplus01@gmail.com'); 
                alert('Adresse email copiée dans le presse-papier : hakimiplus01@gmail.com'); 
              }} 
